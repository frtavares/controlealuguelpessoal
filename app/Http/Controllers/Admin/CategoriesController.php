...
class CategoriesController extends Controller
{
    ...
    public function postCategoryAdd(Request $request, $module){
    	...
    	$messages = [
    		...
    		'icon.required' => 'Campo ícone é requerido!'
    	];

    	$validator = Validator::make($request->all(), $rules, $messages);
    	if($validator->fails()):
    		return back()->withErrors($validator)->with('message', 'Houve um erro!')->with('typealert', 'danger');
    	else:
            $path = '/'.date('d-m-Y');
    ...
    public function getCategoryDelete($id){
        $c = Category::find($id);
        if($c->delete()):
            return back()->with('message', 'Exclusão realizada com sucesso!')->with('typealert', 'success');
        endif;
    }
}
